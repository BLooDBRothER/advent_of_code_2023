...
namespace part2 {
    ...
    void split_card_by_type(vector<string> cards, unordered_map<string, vector<string>>& card_type_map) {
        ...
        for(auto card: cards) {
            ...
            if(card_label_map.contains('J')) {
                int64_t j_card_count = card_label_map['J'];
                card_label_map.erase('J');
                int64_t max = INT64_MIN;
                char max_label;

                for(auto card_label: card_label_map) {
                    if(card_label.second > max) {
                        max_label = card_label.first;
                        max = card_label.second;
                    }
                }
                for(auto &c: temp_card) {
                    if(c == 'J')
                        c = max_label;
                }
                card_label_map[max_label] += j_card_count;
            }
            ...
            switch(card_label_map.size()) {
                case 1:
                    ...
                    break;
                case 2:
                    card_type = (card_label_map.at(temp_card[0]) == 4 || card_label_map.at(temp_card[0]) == 1) ? "four_kind" : "full_house";
                    break;
                case 3:
                    is_three_kind = false;
                    for(auto card_label: card_label_map) {
